...
public:
    BaseNode() : Node("base_node") {
        ...
        timer_ = this->create_wall_timer(std::chrono::milliseconds(100), std::bind(&BaseNode::loop, this));
        
        RCLCPP_INFO(get_logger(), "Base node ready. Relying on hardware flash for UART2 RTCM TX.");
    }
...
private:
    ...
    void handle_pvt(size_t index) {
        ...
        // Only throttle the terminal output to avoid spam
